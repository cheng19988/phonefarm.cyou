...
export async function notifyContactSubmission(data: ContactPayload): Promise<void> {
  ...
  if (!botToken || !chatId) {
    console.warn("[notify] TELEGRAM_BOT_TOKEN or TELEGRAM_NOTIFY_CHAT_ID not configured — inquiry alert skipped");
    return;
  }
  ...
  if (!res?.ok) {
    console.warn("[notify] Telegram inquiry send failed");
  }
}
